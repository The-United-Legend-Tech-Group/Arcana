...
export class AppraisalRecordService {
  ...
  async getAllRecords(query: GetAllRecordsQueryDto): Promise<{
    ...
    if (query.search && query.search.trim()) {
      ...
      filter.$or = [
        {
          employeeProfileId: {
            $in: profileIds.map((id) => new Types.ObjectId(id)),
          },
        },
        {
          managerProfileId: {
    ...
    for (const record of paginatedRecords) {
      if (record.employeeProfileId)
        employeeIds.add(record.employeeProfileId.toString());
      if (record.managerProfileId)
        managerIds.add(record.managerProfileId.toString());
    ...
    const [employees, managers, cycles] = await Promise.all([
      this.employeeProfileRepository.find({
        _id: {
          $in: Array.from(employeeIds).map((id) => new Types.ObjectId(id)),
        },
      }),
      this.employeeProfileRepository.find({
        _id: {
          $in: Array.from(managerIds).map((id) => new Types.ObjectId(id)),
        },
      }),
      this.appraisalCycleRepository.find({
        _id: { $in: Array.from(cycleIds).map((id) => new Types.ObjectId(id)) },
      }),
    ]);
  ...
  async updateRecord(
    ...
    const assignment = await this.appraisalAssignmentRepository.findOne({
      _id: record.assignmentId,
    });
    const isDisputeReEvaluation =
      assignment &&
    ...
    if (isDisputeReEvaluation && assignment) {
      ...
      this.logger.log(
        `Re-evaluation after dispute completed and auto-published. Record ID: ${id}`,
      );
    }

    // Requirement: Downstream Sub-Systems Depending on This Output: Performance Management (Manager ratings and scores)
    this.logger.log(
      `Manager ratings and scores updated for Performance Management consumption. Record ID: ${id}, Total Score: ${totalScore}`,
    );
  ...
  async getFinalizedRecordsForEmployee(
    ...
    const populatedRecords = await Promise.all(
      records.map(async (r) => {
        ...
        if (!overallRatingLabel && template && r.totalScore !== undefined) {
          overallRatingLabel = this.calculateRatingLabel(
            r.totalScore,
  ...
  private calculateRatingLabel(score: number, ratingScale: any): string {
    ...
    for (let i = 0; i < labelCount; i++) {
      ...
      if (
        score >= lowerBound &&
        (i === labelCount - 1 ? score <= upperBound : score < upperBound)
      ) {
  ...
  async createRecord(
    createDto: CreateAppraisalRecordDto,
  ): Promise<AppraisalRecordDocument> {
    // Fetch the template to validate ratings
    const template = await this.appraisalTemplateRepository.findOne({
    ...
    const created = await this.appraisalRecordRepository.create(
      recordPayload as any,
    );
    ...
    // Requirement: Downstream Sub-Systems Depending on This Output: Performance Management (Manager ratings and scores)
    this.logger.log(
      `Manager ratings and scores created for Performance Management consumption. Record ID: ${created._id}, Total Score: ${totalScore}`,
    );
   ...
   */
  private async getContractIdForEmployee(
    employeeProfileId: string,
  ): Promise<string | null> {
    try {
      const employee =
        await this.employeeProfileRepository.findById(employeeProfileId);
      if (!employee || !employee.employeeNumber) {
        this.logger.warn(
          `Employee profile not found or missing employeeNumber: ${employeeProfileId}`,
        );
        return null;
      }
      ...
      if (!candidate) {
        this.logger.warn(
          `Candidate not found for candidateNumber: ${candidateNumber}`,
        );
        return null;
      }

      const contracts = await this.contractRepository.findByCandidateId(
      ...
      this.logger.error(
        `Error getting contract for employee ${employeeProfileId}:`,
        error,
      );
      return null;
    }
  ...
  private async checkAndTriggerTerminationReview(
    ...
    try {
      this.logger.log(
        `[TerminationCheck] Starting check for employee ${employeeProfileId}, currentRecordId: ${currentRecordId}, templateId: ${templateId}`,
      );
      ...
      if (!template) {
        this.logger.warn(
          `[TerminationCheck] Template not found: ${templateId}`,
        );
        return;
      }
      ...
      this.logger.log(
        `[TerminationCheck] Template ratingScale.min: ${minScore}`,
      );

      // Check current record
      let currentIsMin = false;
      const currentRecord = await this.appraisalRecordRepository.findOne({
        _id: currentRecordId,
      });
      if (currentRecord && this.isMinimumScoreRecord(currentRecord, minScore)) {
        currentIsMin = true;
        this.logger.log(
          `[TerminationCheck] Current record ${currentRecordId} is minimum-score`,
        );
      } else {
        this.logger.log(
          `[TerminationCheck] Current record ${currentRecordId} is NOT minimum-score`,
        );
      }
      ...
      this.logger.log(
        `[TerminationCheck] Found ${existingRecords.length} historical HR_PUBLISHED records`,
      );
      ...
      for (const record of existingRecords) {
        ...
        if (isMinScore) {
          minScoreCount++;
          this.logger.log(
            `[TerminationCheck] Historical record ${(record as any)._id} is minimum-score`,
          );
        }
      }

      this.logger.log(
        `[TerminationCheck] Employee ${employeeProfileId} has ${minScoreCount} total minimum-score appraisals (threshold: 3)`,
      );

      // If 3 or more minimum-score appraisals, trigger termination review
      if (minScoreCount >= 3) {
        this.logger.warn(
          `[TerminationCheck] THRESHOLD EXCEEDED! Employee ${employeeProfileId} has ${minScoreCount} minimum-score appraisals. Initiating termination review.`,
        );
        ...
        try {
          ...
          this.logger.log(
            `[TerminationCheck] Notification sent to employee ${employeeProfileId}`,
          );
        } catch (notifError) {
          this.logger.error(
            `[TerminationCheck] Failed to send notification:`,
            notifError,
          );
        }
        ...
        try {
          await this.employeeProfileRepository.updateById(employeeProfileId, {
            status: EmployeeStatus.SUSPENDED,
            statusEffectiveFrom: new Date(),
          });
          this.logger.log(
            `[TerminationCheck] Employee ${employeeProfileId} status updated to SUSPENDED due to repeated poor performance.`,
          );
        } catch (statusError) {
          this.logger.error(
            `[TerminationCheck] Failed to update employee status to SUSPENDED:`,
            statusError,
          );
        }
        ...
        const contractId =
          await this.getContractIdForEmployee(employeeProfileId);

        if (!contractId) {
          this.logger.warn(
            `[TerminationCheck] No contract found for employee ${employeeProfileId}. Termination review cannot be initiated automatically. Manual HR intervention required.`,
          );
        ...
        try {
          await this.offboardingService.initiateTerminationReview({
            ...
            hrComments:
              'Automatically initiated due to repeated poor performance appraisals.',
          });
          this.logger.log(
            `[TerminationCheck] Termination review initiated for employee ${employeeProfileId}`,
          );
        } catch (termError) {
          this.logger.error(
            `[TerminationCheck] Failed to initiate termination review:`,
            termError,
          );
        }
      } else {
        this.logger.log(
          `[TerminationCheck] Threshold NOT met (${minScoreCount} < 3). No action taken.`,
        );
      }
    } catch (error) {
      // Log error but don't fail the main operation
      this.logger.error(
        `[TerminationCheck] Error during check for employee ${employeeProfileId}:`,
        error,
      );
    }
  }
  ...
  async getTeamPerformanceSummary(managerId: string): Promise<{
    ...
    for (const member of teamMembers) {
      ...
      if (scoreData.totalScore !== null) {
        ...
        if (scoreData.ratingLabel) {
          const currentCount =
            scoreDistributionMap.get(scoreData.ratingLabel) || 0;
    ...
    if (reviewedCount > 0) {
      const allRecords = await this.appraisalRecordRepository.find({
        employeeProfileId: {
          $in: teamMembers.map((m: any) => m._id.toString()),
        },
        status: AppraisalRecordStatus.HR_PUBLISHED,
      });
      ...
      if (allRecords.length > 0) {
        const sortedRecords = allRecords.sort((a, b) => {
          const dateA = a.hrPublishedAt
            ? new Date(a.hrPublishedAt).getTime()
            : 0;
          const dateB = b.hrPublishedAt
            ? new Date(b.hrPublishedAt).getTime()
            : 0;
          return dateB - dateA;
        });
    ...
    const scoreDistribution = Array.from(scoreDistributionMap.entries()).map(
      ([label, count]) => ({
        label,
        count,
      }),
    ...
    return {
      averageScore:
        reviewedCount > 0
          ? Math.round((totalScore / reviewedCount) * 100) / 100
          : null,
      topPerformer,
