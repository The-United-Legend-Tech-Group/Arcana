...
describe('TimeService - Shift flows', () => {
	...
	beforeEach(() => {
		mockShiftRepo = {
			create: jest.fn().mockImplementation((dto) => Promise.resolve({ _id: 'shift1', ...dto })),
			find: jest.fn().mockResolvedValue([]),
		};

		mockShiftAssignmentRepo = {
			create: jest.fn().mockImplementation((dto) => Promise.resolve({ _id: 'assign1', ...dto })),
			updateById: jest.fn().mockImplementation((id, update) => Promise.resolve({ _id: id, ...update })),
			find: jest.fn().mockResolvedValue([]),
		};
	...
	it('creates a shift via repository', async () => {
		const dto = { name: 'Morning', shiftType: 'type1', startTime: '08:00', endTime: '17:00' };
	...
	it('updates shift assignment status by id', async () => {
		const res = await service.updateShiftAssignmentStatus('assign1', { status: 'APPROVED' } as any);

		expect(mockShiftAssignmentRepo.updateById).toHaveBeenCalledWith('assign1', { status: 'APPROVED' });
		expect(res).toHaveProperty('_id', 'assign1');
	...
	it('queries assignments for an employee within a term', async () => {
		mockShiftAssignmentRepo.find.mockResolvedValueOnce([
			{ _id: 'a1', employeeId: 'emp1', startDate: new Date('2025-11-05'), endDate: new Date('2025-11-10') },
		]);

		const results = await service.getShiftsForEmployeeTerm('emp1', '2025-11-01', '2025-11-30');
