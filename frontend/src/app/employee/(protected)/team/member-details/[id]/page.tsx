    ...
    React.useEffect(() => {
        const fetchEmployee = async () => {
            ...
            try {
                ...
                // Using the specific endpoint for fetching other employees if available, 
                // or falling back to the generic one if consistent permissions allow.
                // Based on controller, `GET /employee/:id` is protected but might be for self. 
                // However, `GET /employee/team/profiles` worked for the list.
                // Let's try `GET /employee/:id` as it seems to be the general "get profile" endpoint.
                // If that fails due to permissions (accessing other user), we might need a new endpoint or check if the backend allows it.
                // But for now, I'll assume standard access or that managers can view their team.
    ...
    if (error || !employee) {
        return (
            <Box sx={{ p: 4, textAlign: 'center' }}>
                <Typography variant="h6" color="error">{error || 'Member not found'}</Typography>
